...
export class RoundTripComponent extends BaseFlightSearchChallenge implements OnInit {
  public trips: Trip[];
  ...
  ngOnInit(): void {
    super.ngOnInit();
    this.instructions = instructions.roundTrip;
    this.challengeApis = challengesApis.roundTrip;
  ...
  search(originCity: string, destinationCity: string, departureDate: string, returnDate: string): void {
    ...
    this.flightSearchService.roundTrip(journeyRequest).subscribe(trips => this.trips = trips);
  }
